...
func (p Precompile) GetDelegationParamsFromInputs(ctx sdk.Context, args []interface{}) (*delegationtypes.DelegationOrUndelegationParams, error) {
	...
	if !ok {
		return nil, xerrors.Errorf(exocmn.ErrContractInputParaOrType, 1, "uint64", args[1])
	}
	...
	if !ok || stakerAddr == nil {
		return nil, xerrors.Errorf(exocmn.ErrContractInputParaOrType, 3, "[]byte", args[3])
	}
	...
	if !ok || operatorAddr == nil {
		return nil, xerrors.Errorf(exocmn.ErrContractInputParaOrType, 4, "[]byte", args[4])
	}
	if len(operatorAddr) != types.ExoCoreOperatorAddrLength {
		return nil, xerrors.Errorf(exocmn.ErrInputOperatorAddrLength, len(operatorAddr), types.ExoCoreOperatorAddrLength)
	}

	opAccAddr, err := sdk.AccAddressFromBech32(string(operatorAddr))
	if err != nil {
		return nil, xerrors.Errorf("error occurred when parse acc address from Bech32,the addr is:%s, error:%s", string(operatorAddr), err.Error())
	}
	delegationParams.OperatorAddress = opAccAddr

	opAmount, ok := args[5].(*big.Int)
	if !ok || opAmount == nil || !(opAmount.Cmp(big.NewInt(0)) == 1) {
		return nil, xerrors.Errorf(exocmn.ErrContractInputParaOrType, 5, "*big.Int", args[5])
	}
