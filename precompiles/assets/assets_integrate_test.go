...
func (s *AssetsPrecompileSuite) TestWithdrawPrecompileError() {
	// deposit params for test
	exoCoreLzAppAddress := "0x3fC91A3afd70395Cd496C647d5a6CC9D4B2b7FAD"
	exoCoreLzAppEventTopic := "0xc6a377bfc4eb120024a8ac08eef205be16b817020812c73223e81d1bdb9708ec"
	defaultParams := assetstype.Params{
		ExocoreLzAppAddress:    exoCoreLzAppAddress,
		ExocoreLzAppEventTopic: exoCoreLzAppEventTopic,
	}
	...
	prepareFunc := func(params *assetstype.Params, method string, nonce *big.Int, amount *big.Int) contracts.CallArgs {
		err := s.App.AssetsKeeper.SetParams(s.Ctx, params)
		s.Require().NoError(err)
		withdrawArgs := defaultCallArgs.WithMethodName(method)
		withdrawArgs = withdrawArgs.WithNonce(nonce)
		return withdrawArgs.WithArgs(
			uint32(clientChainLzID),
			paddingAssetAddress,
			stakerAddr,
			amount,
	...
	// testDepositTo
	beforeEach()
	defaultParams.ExocoreLzAppAddress = contractAddr.String()
	withdrawArgs := prepareFunc(&defaultParams, "testWithdrawPrincipal", big.NewInt(1), withdrawAmount)
	// check contract was correctly deployed
	cAcc = s.App.EvmKeeper.GetAccount(s.Ctx, contractAddr)
	s.Require().NotNil(cAcc)
	s.Require().True(cAcc.IsContract())
	cosmosResponse, ethResponse, err := contracts.CallContractAndCheckLogs(s.Ctx, s.App, withdrawArgs, passCheck)
	s.Require().NoError(err)
	fmt.Printf("cosmos response: %v\n, ethResponse: %v", cosmosResponse, ethResponse)
	fmt.Printf("error: %v\n", err)
	// todo: need to find why the ethRet is nil when called by contract
	/*	successRet, err := contracts.DepositCallerContract.ABI.Methods["testDepositTo"].Outputs.Pack(true, opAmount)
		s.Require().NoError(err)
	...
	// testCallDepositToAndEmitEvent
	beforeEach()
	withdrawArgs = prepareFunc(&defaultParams, "testCallWithdrawPrincipalWithTryCatch", big.NewInt(2), withdrawAmount)
	// todo: need to check why can't get the ethereum log
	// eventCheck := passCheck.WithExpEvents("callDepositToResult")
	cosmosResponse, ethResponse, err = contracts.CallContractAndCheckLogs(s.Ctx, s.App, withdrawArgs, passCheck)
	s.Require().NoError(err)
