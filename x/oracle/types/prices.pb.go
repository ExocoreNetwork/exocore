...
type Prices struct {
	...
	NextRountId int64 `protobuf:"varint,2,opt,name=next_rount_id,json=nextRountId,proto3" json:"next_rount_id,omitempty"`
	//arrays's index corresponds to the roundID, start from 1
	//  repeated PriceWithTime price_with_round = 3;
	PriceWithRound map[int64]*PriceWithTime `protobuf:"bytes,3,rep,name=price_with_round,json=priceWithRound,proto3" json:"price_with_round,omitempty" protobuf_key:"varint,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}
...
func (m *Prices) GetNextRountId() int64 {
	if m != nil {
		return m.NextRountId
	}
	return 0
}
...
var fileDescriptor_50cc9ccc8f92b87a = []byte{
	...
	,
}
...
func (m *Prices) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.NextRountId != 0 {
		i = encodeVarintPrices(dAtA, i, uint64(m.NextRountId))
		i--
		dAtA[i] = 0x10
	}
	if m.TokenId != 0 {
...
func (m *Prices) Size() (n int) {
	...
	if m.NextRountId != 0 {
		n += 1 + sovPrices(uint64(m.NextRountId))
	}
	if len(m.PriceWithRound) > 0 {
...
func (m *Prices) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NextRountId", wireType)
			}
			m.NextRountId = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.NextRountId |= int64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
